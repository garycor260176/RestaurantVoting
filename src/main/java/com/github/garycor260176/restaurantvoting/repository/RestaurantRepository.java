...
public interface RestaurantRepository extends BaseRepository<Restaurant> {
    ...
    List<Restaurant> findByNameAndAddressIgnoreCase(String name, String address);

    @Query("select r from Restaurant r LEFT JOIN fetch r.dishes d where r.id=?1 and d.dish_date>=?2 and d.dish_date<=?3 order by d.name, d.dish_date DESC")
